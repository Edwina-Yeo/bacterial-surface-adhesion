...
x_int=3.12013; % integral of x^(-1/3) between 0 and 3
...
beta=0;
colour=[1 0.5 0.95];
add_agent_data_all(L,gam,beta_str,beta,Vs,U,D_r,D_r_str,colour,coeff_agent)
...
ylabel('Scaled net bacterial adhesion rate, $\bar{J}$')
...
% loglog(gam(1:6),5e7*gam(1:6).^(1/3)./(2500*750),'k-.',HandleVisibility='off')
% loglog(gam(12:17),9e9*gam(12:17).^(-1 ...
%     )./(2500*750),'k-.',HandleVisibility='off')

loglog(gam(1:6),3e7*gam(1:6).^(1/3)./(2500*750),'k-.',HandleVisibility='off')
loglog(gam(12:17),9e9*gam(12:17).^(-1 ...
    )./(2500*750),'k-.',HandleVisibility='off')
ylim([8e3,11e8]./(2500*750))
 xlim([2e-2,gam(ii+1)])
 title('(b)')
...
function add_agent_data_all(L,gam,beta_str,beta,Vs,U,D_r,D_r_str,colour,coeff)

% Scale factor S(\beta,Per,Vss) eq 18.
factor=@(b,per,Vss) (32 + (2- b)*(1- b)*per.^2)./(16 + (4 - b^2)*per.^2).*Vss.^2 ;
ii=17; % Number of points plotted 
...
Pe_effs=(16 + (4 - beta^2)*Pers.^2).*Vss.^2 ./(32 + (2- beta)*(1- beta)*Pers.^2).*(16+Pers.^2)./(4*Vss.^2.*Pers);
% max_eps=max(Pe_effs.^(-1/3))
mean_eps=mean(Pe_effs.^(-1/3))
epss=Pe_effs.^(-1/3);
% min_eps=min(Pe_effs.^(-1/3))
subplot(1,2,1)
% Dimensional plot
