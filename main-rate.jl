...
function run_sim(Pe_r, Vs, gamma, kappa)
    ...
    while num_introduced < target
        ...
        for i in 1:block_size
            swim_x[i] += (dt * (swim_y[i] + Vs * cos(swim_theta[i])))
            swim_y[i] += (dt * (Vs * sin(swim_theta[i])))
            swim_theta[i] += dt * (-0.5 + 0.5 * beta * cos(2 * swim_theta[i])) + D_step * randn()
...
for i in 1:num_params
    ...
    Vs = Vss[i]
    Pe_r = Pers[i]
    stick_events = run_sim(Pe_r, Vs, gamma, kappa)
...
writedlm("data-rates/Vs$Vs_val-beta-$beta-D_r-$D_r-rep-$rep-rate.txt", rates, "   ")
