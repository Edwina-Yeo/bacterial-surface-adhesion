...
x = np.linspace(0.0001, 3, 1000)
y = np.linspace(0.0001, 0.16, 500)

# x = np.linspace(0.01, 3, 300)
# y = np.linspace(0.01, 0.4, 300)

x = np.linspace(0.0001, 3, 1000)
y = np.linspace(0.0001, 0.15, 300)
...
cs = plt.contour(X[1:, 1:], Y[1:, 1:], gaussian_filter(rho, sigma=3), [0.5])
p = cs.collections[0].get_paths()[0]
v = p.vertices
x_a = v[:, 0]
y_a = v[:, 1]
...
fig2=plt.figure()
cs = plt.contour(X_cont, Y_cont, func(X_cont,Y_cont,eps), [0.5],color='none')
...
im0=ax.pcolormesh(X_cont, Y_cont, func(X_cont,Y_cont,eps), cmap='coolwarm',vmin=0, vmax=1.1)
print(rho[:,:])
